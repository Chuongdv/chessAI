...
def getPieceValue(game, square):
    ...
    def getAbsoluteValue(piece, isWhite, square):
        ...
        if piece.symbol().lower() == 'p':
            return 100 + (table.pawnEvalWhite[row][col] if isWhite else table.pawnEvalBlack[row][col]);
        elif piece.symbol().lower() == 'n':
            return 320 + table.knightEval[row][col];
        elif piece.symbol().lower() == 'b':
            return 330 + (table.bishopEvalWhite[row][col] if isWhite else table.bishopEvalBlack[row][col]);
        elif piece.symbol().lower() == 'r':
            return 500 + (table.rookEvalWhite[row][col] if isWhite else table.rookEvalBlack[row][col]);
        elif piece.symbol().lower() == 'q':
            return 900 + table.evalQueen[row][col];
